  ...
  return (
    ...
    <td className={styles.td}>
      <span
        ...
        )}>{product.term}</span></td>

    {direction === 'departure' && <td className={styles.td}>{localTime(product.timeBoard)}</td>}

    {direction === 'arrival' && <td className={styles.td}>{localTime(product.timeArrShedule)}</td>}
    ...
    <td className={styles.td} style={{paddingLeft:'50px'}}>{product.codeShareData[0].codeShare}</td>
    <td className={styles.td}><Link to={`/${direction}/${product.ID}?dt=${date}`} className={styles.details}>Flight details</Link></td>
  </tr>
  );
};
